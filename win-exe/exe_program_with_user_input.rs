...
fn main() -> Result<()> {
    ...
    // get user input of desired work to do

    use std::io::{stdin,stdout,Write};

    let mut input = String::new();

    print!("Enter iterations to do: ");
    let _ =stdout().flush();
    stdin().read_line(&mut input).expect("Did not enter a correct string");


    let work_to_do: usize = input.trim().replace("_", "").parse().expect("Please type a number!");
    ...
    let tests: usize = input.trim().replace("_", "").parse().expect("Please type a number!");
    ...
    let work_vec: Vec<usize> = {        
        ...
        let work_per_worker = work_to_do / total_workers as usize;

        for _ in 0..total_workers {
            output.push(work_per_worker);
        }

        let remainder = work_to_do % total_workers;
